...
@bot.message_handler(commands=['che_si_mangia_oggi'])
def che_si_mangia_oggi(message):
    msg = funzioni.genera_messaggio_piano_alimentare(MEALIE_HOSTNAME, MEALIE_PORT)    
...
@bot.message_handler(commands=['get_scheduled_messages'])
def get_scheduled_messages(message):
   msg = str(schedule.get_jobs())
   bot.send_message(
       message.chat.id,
       msg
   )
...
def scheduler_initialize():
    ...
    schedule.every().monday.at("09:00").do(send_scheduled_turni)
