...
def get_recipes_by_categories(categories, require_all_categories, api_url=mealie_url, token=api_token):
    headers = get_headers(token)
    params = {"perPage": 10**2, "categories": categories,"requireAllCategories":require_all_categories}
    response = requests.get(
        "{}{}".format(api_url, base_url), headers=headers, params=params
    )
